...
from physical_model import modact_forward_model, physics_evaluation_module
import argparse
...
def main(index_test, ground_truth_state):
    ...
    parser.add_argument('--seed_size', type=int, default=128, help='number of seed size')
    ...
    parser.add_argument('--state_meaning', type=list,
                        default=["mff", 'R',
                                 'Zx11', 'Zx12', 'm1', 'b1', 'd1', 'gama1',
                                 'Zx21', 'Zx22', 'm2', 'b2', 'd2', 'gama2',
                                 'Zx31', 'Zx32', 'm3', 'b3', 'd3', 'gama3'], help='state meaning')
    ...
    forward_physical_model = modact_forward_model("cs1")
    args.feasible_domain_state = forward_physical_model.feasible_domain
    actual_observation, actual_constraint = forward_physical_model.generate_sample(
    ...
    if error_first > args.error_threshold:
        ...
        error_estimator, state_optimizer, error_estimator_optimizer, mse_loss, seed_for_RAM = \
            optimization_network_config(LR_RAM=1.e-2, LR_error=1.e-4, LR_sample=1e-3, state_size=state_size,
        ...
        for i in range(0, args.num_iteration):
            ...
            loss_state, estimated_error, est_state_norm, valid_flag = RAM_net_training_inference(RAM_net_optimizer=state_optimizer,
...
if __name__ == '__main__':
    ground_truth = np.loadtxt("X_choose_list.csv")
